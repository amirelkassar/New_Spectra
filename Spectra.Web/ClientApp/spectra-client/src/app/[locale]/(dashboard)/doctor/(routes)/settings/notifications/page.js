...
const NotificationIcon = ({ type }) => {
  const icon = useMemo(() => {
    switch (type) {
      case 'transfer':
        return (
          <TransfersIcon fill='#10B0C1' className={'w-full h-auto'} />
        );
      case 'team':
      ...
      case 'accept':
        return (
          <CircleCheck className={'w-full h-auto text-greenMain'} />
        );
      default:
  ...
  return (
    <div className='text-xl max-w-6  w-full h-auto text-blue-500 '>
      {icon}
    </div>
  );
};
