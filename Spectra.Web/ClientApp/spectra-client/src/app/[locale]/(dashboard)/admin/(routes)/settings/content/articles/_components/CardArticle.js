...
function CardArticle({ data }) {
  return (
    <div className='flex justify-between gap-3 pb-8 border-b border-grayLight'>
      <Link
        href={ROUTES.ADMIN.SETTINGS.CONTENT.EDITARTICLES(data.id)}
      ...
      >
        ...
        <div>
          ...
          <p className='text-[12px] font-Regular text-grayDark'>
            {data.date}
          </p>
      ...
      <div>
        <MenuActions
          type={2}
          path={ROUTES.ADMIN.SETTINGS.CONTENT.EDITARTICLES(data.id)}
          pathEdit={ROUTES.ADMIN.SETTINGS.CONTENT.EDITARTICLES(
            data.id
          )}
        />
      </div>
