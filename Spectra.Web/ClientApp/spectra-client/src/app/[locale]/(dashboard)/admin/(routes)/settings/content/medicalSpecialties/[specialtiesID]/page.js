  ...
  const getStar = (num) => {
    ...
    for (let i = 1; i <= num; i++) {
      stars.push(
        <StarGoldIcon
          key={i}
          className={'w-[10px] lg:w-[18px] h-auto'}
        />
      );
    }
    return stars;
    ...
    <Card>
      <div className=' mb-7 lgl:mb-14 '>
        ...
        <p className='text-center lgl:text-start text-grayDark my-4 lgl:my-8 text-sm lgl:text-[18px] font-Regular'>
          يرجى تحديد الأطباء الذين ترغب في ظهورهم على الموقع الذي
          يتصفحه العملاء و الزوار
        </p>
      </div>
      <div>
        <Table>
              ...
              <Table.Tr key={index} className='mt-3'>
                <Table.Td className='flex items-center gap-2  lgl:gap-4'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    />
                    <h2 className='text-[12px] lgl:text-base min-w-[96px]'>
                      {doctor.name}
                    </h2>
                  </div>
                </Table.Td>
                <Table.Td className='text-[12px] lgl:text-base'>
                  {doctor.specialisationDoctor}
                </Table.Td>
                <Table.Td className='text-[12px] lgl:text-base'>
                  {doctor.date}
                </Table.Td>
