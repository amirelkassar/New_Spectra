      ...
      >
        ...
        <div className='flex-1 flex flex-col gap-y-1 gap-x-5 mdl:flex-row mdl:*:flex-1'>
          <div className=''>
            ...
            <p className='text-xs mdl:text-base'>
              {locale === 'ar' ? 'الطبيب' : 'Doctor'} {data?.doctor}
            </p>
          </div>
        ...
        {clickable && (
          <div className='ltr:rotate-180 size-8 mdl:size-12 flex items-center justify-center bg-blueLight rounded-full shrink-0'>
            <ArrowLeft fill='#10B0C1' className='mdl:w-6 w-4' />
          </div>
        )}
      </Card>
    </div>
...
const SessionInfo = ({ locale = 'ar', data = {}, onBack }) => {
  return (
      ...
      >
        ...
        <div className='space-y-3'>
          ...
          <div className='grid grid-cols-2 xl:grid-cols-3 gap-5'>
            {packagesDataSpectra?.slice(0, 2)?.map((p, i) => (
              <PackageCard
        ...
        <div className='space-y-3'>
          ...
          <div className='grid grid-cols-2 xl:grid-cols-3 gap-5'>
            <Service
              ...
              icon={<SessionIcon className='size-8 mdl:size-12' />}
            />
          </div>
        </div>
      </Card>
...
const Service = ({
  ...
  return (
    ...
    >
      ...
      <Button
        variant={status === 'reserved' ? 'blueLight' : 'secondary'}
      ...
      <div className='grid grid-cols-2 xl:grid-cols-3 gap-5'>
        {data?.map((item, i) => (
          <Attachment key={i}>
            <Attachment.Icon />
            <div>
              <Attachment.Name>{item?.fileName}</Attachment.Name>
...
const SessionReport = ({ reportTitle = '', reportDetails = [] }) => {
  return (
