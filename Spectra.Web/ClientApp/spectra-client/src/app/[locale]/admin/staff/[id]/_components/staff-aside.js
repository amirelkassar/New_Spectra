...
const StaffAside = () => {
  ...
  const pages = [
    {
      name: "الـمواعيد ",
      route: ROUTES.ADMIN.STAFF.STAFFID(params.id)+'?type='+searchParams.get("type"),
      isActive: path === ROUTES.ADMIN.STAFF.STAFFID(params.id),
    },
    {
      name: "الوصفات الطبية ",
      route: ROUTES.ADMIN.STAFF.STAFFIDPRESCRIPTIONS(params.id)+'?type='+searchParams.get("type"),
      isActive: path === ROUTES.ADMIN.STAFF.STAFFIDPRESCRIPTIONS(params.id),
    },
 
    {
      name: "الـمرضى ",
      route: ROUTES.ADMIN.STAFF.STAFFIDCLIENTS(params.id)+'?type='+searchParams.get("type"),
      isActive: path === ROUTES.ADMIN.STAFF.STAFFIDCLIENTS(params.id),
    },
    {
      name: "معلومات",
      route: ROUTES.ADMIN.STAFF.STAFFIDINFORMATION(params.id)+'?type='+searchParams.get("type"),
