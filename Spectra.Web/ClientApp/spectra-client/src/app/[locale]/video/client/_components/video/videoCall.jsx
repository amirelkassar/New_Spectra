...
export const VideoCall = () => {
  ...
  return (
    <div className='relative aspect-video overflow-hidden lgl:rounded-xl'>
      ...
      <Host className='lgl:max-w-52 max-w-32' />

      <TagName>الاستشاري احمد محمد</TagName>
      ...
      <MobileFullScreenToggle className='absolute sml:hidden cursor-pointer bottom-1 z-10 start-1/2 translate-x-1/2 ltr:-translate-x-1/2' />
    </div>
  );
};
...
const Host = ({ ...props }) => {
  return (
    <div
      {...props}
      className={cn(
        'relative aspect-video border-4 border-white rounded-xl overflow-hidden z-10 top-2 start-2',
        props?.className
      )}
    >
      <Image
...
const CallDuration = ({ children, ...props }) => {
  return (
    <div
      {...props}
      className={cn(
        'lgl:w-24 w-20 lgl:h-10 h-8 bg-white/35 rounded-full p-2 flex items-center justify-center font-medium text-sm border border-white text-[#2B2B2B] absolute top-2 start-1/2 translate-x-1/2 ltr:-translate-x-1/2',
        props?.className
      )}
      style={{
...
const TagName = ({ children, ...props }) => {
  return (
    <div
      {...props}
      className={cn(
        'rounded-lg bg-white/35 text-sm lgl:text-xl font-bold py-1 px-4 w-fit absolute top-2 end-2',
        props?.className
      )}
      style={{
        backdropFilter: 'blur(11px)',
