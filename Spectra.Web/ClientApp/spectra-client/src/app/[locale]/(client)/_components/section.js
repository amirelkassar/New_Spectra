...
export const Section = ({
  ...
  return (
    <section role='region' {...props} className='my-16 w-full'>
      {type === 'basic' && (
        <h2 id={id} className='text-2xl text-black leading-9 text-start mb-10'>
          {heading}
        </h2>
      )}
      {type === 'both' && (
      ...
      {type === 'button' && (
        <div className='w-full relative mb-10'>
          <Button className='flex items-center absolute left-0 top-0 text-black font-bold leading-6'>
            <span>{btnLabel}</span>
            <ArrowLeft />
          </Button>
        </div>
