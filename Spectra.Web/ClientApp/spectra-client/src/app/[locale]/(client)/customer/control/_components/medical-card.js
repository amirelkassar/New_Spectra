...
const data = [
  {
    ...
    number: 1002,
    percentage: 40,
    icon: <BarsIcon className='w-5 lg:w-auto' />,
    color: 'bg-red/10',
  },
  {
    label: 'الاستشارات',
  ...
  },
  {
    ...
    number: 1250,
    percentage: -10,
    icon: <CalenderIcon className='w-5 lg:w-auto' />,
...
const Card = ({
  ...
  return (
    <div
      ...
      className='bg-white flex-1 w-fit max-w-full space-y-3 py-3 px-8 rounded-[10px] min-w-[213px] lg:min-w-max'
    >
      {/* ICON AND LABEL */}
      <div className='flex items-center gap-x-3'>
        {/* ICON */}
        <div
          className={`size-10 shrink-0 lg:size-16 rounded-full flex items-center justify-center ${color}`}
        >
          {icon}
        </div>
        {/* LABEL */}
        <h3 className='text-black lg:text-medium text-sm font-normal'>
          {label}
        </h3>
      </div>
      ...
      <div className='flex items-center gap-x-3'>
        {/* NUMBER */}
        <p className='text-black lg:text-[28px] leading-10 text-base font-bold'>
